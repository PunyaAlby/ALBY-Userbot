...
@register(outgoing=True, pattern=r"^\.(?:lope)\s?(.)?")
...
@register(outgoing=True, pattern=r"^\.(?:lope1)\s?(.)?")
async def emoji_lope1(e):
    emoji = e.pattern_match.group(1)
    lope1 = GAMBAR_LOPE1
    if emoji:
        lope1 = lope1.replace('😂', emoji)
    await e.edit(lope1)


@register(outgoing=True, pattern=r"^\.(?:gembok)\s?(.)?")
async def emoji_gembok(e):
    emoji = e.pattern_match.group(1)
    gembok = GAMBAR_GEMBOK
    if emoji:
        gembok = gembok.replace('😂', emoji)
    await e.edit(gembok)
