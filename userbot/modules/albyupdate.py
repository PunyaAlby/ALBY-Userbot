...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await event.edit(
                f'{txt}\n`Kredensial Heroku tidak valid untuk deploy ALBY-USERBOT dyno.`'
            )
            return repo.__del__()
...
async def update(event, repo, ups_rem, ac_br):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, "#BOT \n"
            "**ALBY-USERBOT Telah Di Perbarui ツ**")
...
async def upstream(event):
    ...
    if force_update:
        ...
        await event.edit(f"`Proses Update {REPO_NAME}, Loading....1%`")
        await event.edit(f"`Proses Update {REPO_NAME}, Loading....20%`")
        await event.edit(f"`Proses Update {REPO_NAME}, Loading....35%`")
        await event.edit(f"`Proses Update {REPO_NAME}, Loading....77%`")
        await event.edit(f"`Proses Update {REPO_NAME}, Updating...90%`")
        await event.edit(f"`Proses Update {REPO_NAME}, Mohon Menunggu....100%`")
    if conf == "now":
