...
ALBY_TEKS_KUSTOM = os.environ.get("ALBY_TEKS_KUSTOM", "")
        ...
        async def opeen(event):
            try:
                ...
                if tgbotusername is not None:
                    results = await event.client.inline_query(tgbotusername, "@albyuserbot")
...
# ====================================InlineHandler===================================== #
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid:
                text = (
                    f"❁ __Saya Adalah ALBY Userbot Yang Digunakan Banyak User Telegram__.\n\n"
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid:
                text = (
                    ...
                    f"× **Pembaruan Data Untuk ALBY Userbot, Command Untuk Pembaruan**.\n"
                    f"⚒Pembaruan Data :\n"
                    f"`.update deploy`\n"
                    f"`update`\n\n"
                    f"© @Punya_Alby")
