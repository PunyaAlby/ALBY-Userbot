...
async def get_user_from_event(
    event, albyevent=None, secondgroup=None, nogroup=False, noedits=False
):
    if albyevent is None:
        albyevent = event
    if nogroup is False:
        if secondgroup:
    ...
    try:
        ...
        if event.reply_to_msg_id:
            ...
            if previous_message.sender_id is None:
                if not noedits:
                    await edit_delete(
                        albyevent, "**ERROR: Dia adalah anonymous admin!**", 60
                    )
                return None, None
