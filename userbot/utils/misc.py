...
async def animator(media, mainevent, textevent):
    ...
    Alby = await mainevent.client.download_media(media, TEMP_DOWNLOAD_DIRECTORY)
    await textevent.edit("`Converting...`")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {Alby} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an Video.webm"
    )
    os.remove(Alby)
    vid = "Video.webm"
    return vid
